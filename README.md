...
方案排序會優先考慮直達，再考慮實際轉乘與其他分段類型。只要指定日期與出發時段內仍有不含「座位有限」的方案，購票建議就不會列出座位有限方案；只有沒有其他可行方案時，座位有限才會作為兜底結果。自由座只代表 TDX 回報該車次有自由座車廂，不保證現場一定有座位；每個方案的購票步驟仍需在官方通路重新確認。

查詢與購票建議條件可使用「已儲存條件」管理。純查詢選取已儲存條件時會原樣帶入日期；若是每天通勤等固定條件，可按「帶入並改為今天」，將所有行程日期快速設為今天，而不會修改原本儲存的條件。條件儲存在瀏覽器的 `localStorage`，剪貼簿功能匯出的內容是 JSON，不會上傳到後端。
  ...
  "results": [
    {
      ...
      "tdxUpdatedAt": "2026-09-10T02:59:50+08:00",
      "sourceUpdatedAt": "2026-09-10T02:59:42+08:00"
    },
    {
      "date": "2026-09-21",
      "seats": [],
...
`cachedAt` 是本網站本地抓取／快取時間；`tdxUpdatedAt` 是 TDX 平台更新時間；`sourceUpdatedAt` 是高鐵來源資料更新時間。三者不同時，不代表高鐵票況在本網站抓取時間點剛好更新。
...
本專案使用高鐵的起訖站 OD 座位資料產生方案，不直接把原始列車區段 Leg 資料當成 OD 結果。若改用原始 Leg API，必須另外考慮起站與途中上車站的座位狀態；下車站本身不需判斷座位是否有餘位。實際購票仍以高鐵官方訂票系統為準。
...
- 座位使用整日 OD API，時刻表使用整日所有車次 API；純查詢與購票規劃依日期共用快取，透過本機索引比較起迄站與中間站。OD 座位結果中的 `SrcUpdateTime`／`UpdateTime` 會保留為 API 回應的 `sourceUpdatedAt`／`tdxUpdatedAt`。
...
- 依 TDX 高鐵資料規格，若高鐵營運資料異常，剩餘座位可能停留在最後正常資料；此時應以高鐵官方訂票系統或車站現場看板為準。
